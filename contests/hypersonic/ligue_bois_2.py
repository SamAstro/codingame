...
"""
BOT FOR LIGUE BOIS 2 OF HYPERSONIC CONTEST
        ...
        if oidx == None:
            ...
            if (disty < bomb_reach or distx < bomb_reach):
                if entity_dict[myidx].param_1 != 0:
                    ...
                    if (entity_dict[myidx].x+offset > width-1) or (entity_dict[myidx].y+offset > height-1):
                        print("MOVE", entity_dict[myidx].x-5, entity_dict[myidx].y-5, sep=" ")
                    else:
                        print("MOVE", entity_dict[myidx].x+offset, entity_dict[myidx].y+offset, sep=" ")
            else:
